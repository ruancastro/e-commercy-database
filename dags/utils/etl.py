...
class ETLBase(ABC):
    ...
    def extract_oltp(self):
        ...
        df_purchases_items = pd.read_sql("SELECT * FROM purchases_items;", self.oltp_engine)
...
class ETLInitial(ETLBase):
    ...
    def transform(self, extracted_data):
        ...
        dim_time = dim_time[['date_id', 'date', 'day', 'month', 'quarter', 'year', 'is_weekend']]
        if dim_time['date'].isna().any():
            raise ValueError("Null or invalid values found in 'order_date' while creating dim_time")
        ...
        fact_sales = pd.merge(fact_sales, dim_time[['date_id', 'date']], left_on='order_date', right_on='date', how='inner')
        ...
        fact_inventory['date_id'] = dim_time[dim_time['date'] == current_date]['date_id'].iloc[0]
    ...
    def load(self, transformed_data):
        ...
        for table_name in load_order:
            ...
            if df is not None and not df.empty:
                df.to_sql(table_name, self.olap_engine, if_exists='append', index=False, method='multi')
...
class ETLIncremental(ETLBase):
    ...
    def transform(self, extracted_data):
        ...
        new_dates_df = pd.DataFrame({
            ...
            'quarter': new_dates.apply(lambda x: ((x.month - 1) // 3) + 1 if pd.notnull(x) else None),
            'year': new_dates.apply(lambda x: x.year if pd.notnull(x) else None),
            'is_weekend': new_dates.apply(lambda x: x.weekday() in [5, 6] if pd.notnull(x) else None)
        })
        ...
        fact_inventory = df_items_sizes
        fact_inventory = pd.merge(fact_inventory, df_inventory, on=['item_id', 'size_id'], how='right')
        fact_inventory = pd.merge(fact_inventory, dim_time[['date_id', 'date']], left_on=pd.to_datetime(current_date).date(), right_on='date', how='left')
        ...
        for table_name in load_order:
            ...
            if df is not None and not df.empty:
                if table_name == "dim_time":
                    existing_dates = pd.read_sql("SELECT date FROM dim_time", self.olap_engine)['date']
                    new_dates = df[~df['date'].isin(existing_dates)]
                    if not new_dates.empty:
                        new_dates.to_sql(table_name, self.olap_engine, if_exists='append', index=False, method='multi')
                else:
                    df.to_sql(table_name, self.olap_engine, if_exists='append', index=False, method='multi')
