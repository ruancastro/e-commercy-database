...
DATABASE_URL = (
    "postgresql+psycopg2://oltp:ecommerce123@postgres_oltp:5432/ecommerce_oltp"
)
...
class Customer:
    ...
    def create_new_customer(self):
        ...
        customer_result = self.session.execute(
            text(
                "INSERT INTO customers (full_name, email) VALUES (:full_name, :email) RETURNING customer_id"
            ),
            customer_data,
        )
        ...
        address_data = {
            ...
            "complement": (
                None
                if random.random() < P_NULL_COMPLEMENT
                else generate_brazilian_address_complement()
            ),
        ...
        self.session.execute(
            text(
                "INSERT INTO customers_addresses (customer_id, address_id) VALUES (:customer_id, :address_id)"
            ),
        ...
        if random.random() >= P_NULL_PHONE:
            ...
            self.session.execute(
                text(
                    "INSERT INTO phones_customers (phone_id, customer_id) VALUES (:phone_id, :customer_id)"
                ),
...
class Purchase:
    ...
    def create_purchase(self, customer_id, store_id, items_sizes_quantities):
        order_date = fake.date_between(
            start_date=date(2020, 10, 31), end_date=date.today()
        )
        purchase_data = {
...
class EcommerceManager:
    ...
    def register_purchases_and_customers(self):
        purchase_count = self.session.execute(
        ...
        for _ in range(num_purchases):
            ...
            for _ in range(num_items):
                item_size_row = self.session.execute(
                    text(
                        "SELECT item_id, size_id FROM items_sizes ORDER BY RANDOM() LIMIT 1"
                    )
                ).fetchone()
                ...
                if combination in combination_to_index:
                    ...
                    items_sizes_quantities.append(
                        {"item_id": item_id, "size_id": size_id, "quantity": quantity}
                    )
            ...
            for item in items_sizes_quantities:
                ...
                if self.inventory.check_stock(item_id, size_id, store_id, quantity):
                    ...
                    print(
                        f"Insufficient stock for item {item_id}, size {size_id}, store {store_id}, quantity {quantity}. Item skipped."
                    )

            if valid_items:
                purchase_id = self.purchase.create_purchase(
                    customer_id, store_id, valid_items
                )
                for item in valid_items:
                    self.inventory.decrement_stock(
                        item["item_id"], item["size_id"], store_id, item["quantity"]
                    )
            else:
                print(
                    f"No items available with sufficient stock for the purchase in store {store_id}."
                )
